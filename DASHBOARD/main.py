...
for entry in data:
    ...
    for country_data in entry.get("countries", []):
        if country_data.get("country") == "France":
            rows.append(
                {"year": year, "country": country_data.get("country"), "count": 0}
            )
        else:
            rows.append(
                {
...
for df_tmp in [df1, df2]:
    ...
    df_tmp["publicationDate_s"] = pd.to_datetime(
        df_tmp["publicationDate_s"], errors="coerce"
    )
    ...
    for entry in data:
        if entry["name"].lower() == professor_name.lower():
            for result in entry.get("results", []):
                ...
                if (
                    publication_year.isdigit()
                    and start_year <= int(publication_year) <= end_year
                ):
...
if page == "Page 1":
    ...
    university_counts = analyze_data(
        university_data, selected_dashboard_researcher, start_year, end_year
    )
